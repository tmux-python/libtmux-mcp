...
_BASE_INSTRUCTIONS = (
    ...
    "JSON ARGUMENT FORMAT: every tool argument must be valid JSON. "
    "String values — including tmux IDs like pane_id "
    "('%1'), session_id ('$10'), window_id ('@5'), and bare names "
    "like session_name ('cv') — must be enclosed in double quotes. "
    'Emit {"pane_id": "%1"}, never {"pane_id": %1}. Unquoted string '
    "values are rejected at the MCP transport before the server runs "
    "and the failure is not recoverable server-side. See "
    "tmux-python/libtmux-mcp#17.\n\n"
