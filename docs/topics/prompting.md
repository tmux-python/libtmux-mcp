...
4. **Content vs. metadata**: If looking for text *in* a terminal, use {toolref}`search-panes`. If looking for pane identity, title, or current command, use {toolref}`list-panes`; request `detail="full"` for PID or path metadata, or use {toolref}`get-pane-info` for one known pane
